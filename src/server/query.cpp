...
//Use set_intersection algorithm to compute the intersection of sets
...
// subset witness is built as follow for each set
...
Union::Union(const std::vector<int> indices, PublicKey *pk, DataStructure *dataStructure) {
    ...
    this->pk = pk;
    this->dataStructure = dataStructure;
    // Tree pointers is a 2D vector, creating the leaves as follow
...
void Union::unionSets() {
    ...
    // build each level on top of the previous level
    while (len > 1) {
        depth++;
        std::vector<QueryNode> tmp;
        tree.push_back(tmp);
...
/* Calculate needed value for each node
 * HU is the digest of Union, HI is the digest of intersection
 * SET is the result of query of childs (union or intersection)
 * F1 and F2 are accumulation values generated by g1 and g2 respectively
*/
...
void Union2::membership_witness() {
    ...
    for (it = U.begin(); it != U.end(); it++) {
        ...
        tmp.push_back(*it);
        // the set index of y = *it is precomputed in datastructure as a map so we don't need to calculate it again
